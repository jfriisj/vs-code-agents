...
4. **Mandatory Planka Exit Gate (QA)**:
  ...
  - Run `card:get` and verify your QA verdict comment exists and your `QA & Testing` tasks are closed.
   - Verify QA tasks created in this phase include `ISS-` IDs when issue decomposition exists.
   - Verify QA verdict comment includes issue coverage and QA artifact link.
