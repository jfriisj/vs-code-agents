...
Resolve runtime context before process-improvement work:
...
3. `RETROSPECTIVE_ROOT`: first existing of `agent-output/retrospectives/`, `retrospectives/`, `docs/retrospectives/`; default `agent-output/retrospectives/`.
4. `PROCESS_IMPROVEMENT_ROOT`: first existing of `agent-output/process-improvement/`, `process-improvement/`, `docs/process-improvement/`; default create/use `agent-output/process-improvement/`.
5. `AGENTS_ROOT`: first existing of `.github/agents/`, `agents/`; default `.github/agents/`.
...
- Only create artifacts in `PROCESS_IMPROVEMENT_ROOT`
...
1. Read retrospective from `RETROSPECTIVE_ROOT`
...
4. Create analysis document: `PROCESS_IMPROVEMENT_ROOT/NNN-process-improvement-analysis.md`
...
1. Update agent instructions using targeted, batched edits across affected `.agent.md` files
...
Create `PROCESS_IMPROVEMENT_ROOT/NNN-process-improvement-analysis.md` with:
...
Create `PROCESS_IMPROVEMENT_ROOT/NNN-agent-instruction-updates.md` with:
...
**Self-check on start**: Before starting work, scan `PROCESS_IMPROVEMENT_ROOT` for docs with terminal Status outside `closed/`. Move them to `closed/` first.
...
4. **Issue Coverage Exit Gate (PI)**:
   - Run `card:get` and verify PI summary comment includes issue-pattern evidence and analysis artifact link.
...
2. **Establish the Upward Edge**: Set frontmatter `type: ProcessImprovement`. Set `parent: "[[WF-Retrospective-ID]]"` using the ID provided by the Retrospective agent.
