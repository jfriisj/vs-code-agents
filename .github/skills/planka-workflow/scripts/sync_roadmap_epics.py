...
def derive_project_name_from_roadmap(roadmap_path: Path) -> str:
    lines = roadmap_path.read_text(encoding="utf-8").splitlines()
    ...
    try:
        repo_name = roadmap_path.parents[2].name.strip()
    except IndexError:
        repo_name = ""

    if repo_name:
        normalized = repo_name.replace("-", " ").replace("_", " ").strip()
        if normalized:
            return normalized
...
def ensure_lists(runner: SyncRunner, board_id: str) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    if missing and runner.dry_run:
        for list_name in missing:
            ...
            list_id_by_name[key] = f"list-dryrun-{key.replace(' ', '-')}"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Planka project name. If omitted, derives from the first H1 in the roadmap "
...
def main() -> None:
    ...
    explicit_project_name = (args.project_name or "").strip()
    resolved_project_name = explicit_project_name or derive_project_name_from_roadmap(roadmap_path)

    if not resolved_project_name:
        raise RuntimeError("Could not resolve a valid project name for Planka synchronization")
